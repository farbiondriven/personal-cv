%-------------------------------------------------------------------------------
%	SECTION TITLE
%-------------------------------------------------------------------------------
\cvsection{Work Experience}


%-------------------------------------------------------------------------------
%	CONTENT
%-------------------------------------------------------------------------------
\begin{cventries}

%---------------------------------------------------------
  %% Spire
  \cventry
  {Senior Software Engineer} % Job title
  {Spire Global} % Organization
  {Luxembourg, Luxembourg} % Location
  {Dec. 2022 - Present} % Date(s)
  {
    \begin{cvitems} % Description(s) of tasks/responsibilities
      \item {Working for the Space Services business unit;}
      \item {Design and development of services to manage customised satellite constellations;}
      \item {Development of Microservices using \textbf{FastAPI, Flask, PostgreSQL, Terraform, Docker, Kubernetes, Github CI/CD actions };}
      \item {Lead and delivered a 2 years development project co-funded by the European Space Agency (ESA)}
    \end{cvitems}
  }
    %% Remark AI
   \cventry
    {Lead Software Engineer} % Job title
    {Remark AI} % Organization
    {London,  United Kingdom} % Location
    {Sep. 2020 - Oct. 2022} % Date(s)
    {
      \begin{cvitems} % Description(s) of tasks/responsibilities
        \item {Design and development of an application for thermal scanning,
        mask wearing recognition and \textbf{face recognition} using thermal or basic network cameras;}
        \item {Development of Microservices using \textbf{Flask, FastAPI, C++, MongoDB, MySQL, GraphQL, Kafka,
        Traefik, Docker, Kubernetes, Gitlab CI/CD Pipelines deploying in AWS or on-premise installations};}
        \item {Integration of AI Pipelines developed using \textbf{DeepStream} framework using C++, Python and Kafka;}
      \end{cvitems}
    }

    %% Amadeus
    \cventry
    {Senior Software Engineer} % Job title
    {Amadeus} % Organization
    {London,  United Kingdom} % Location
    {Sep. 2017 - Sep. 2020} % Date(s)
    {
      \begin{cvitems} % Description(s) of tasks/responsibilities
        \item {I joined the department developing Amadeus Altéa Departure
        Control System (DCS), a software suite which addresses Departure Control activities and
        Customers Management, supporting the whole passenger flow from Check In to Boarding;}
        \item {Design and Development of server applications using \textbf{C++ 11};}
      \end{cvitems}
    }

    % XID
    \cventry
    {Software Engineer} % Job title
    {XiD Technologies} % Organization
    {Singapore} % Location
    {Feb. 2015 - Aug. 2017} % Date(s)
    {
      \begin{cvitems} % Description(s) of tasks/responsibilities
        \item {Design and Development of applications connecting 3rd party devices performing Face and Fingerprint recognition to
        \textbf{XiD FaaS system, integrating Hanvon, ZK, Suprema} with custom Business requirements;}
        \item {Integrated Fingerpint and RFID Security System and CCTV for \textbf{Changi Airport Terminal 5 construction}, providing
        real-time screen alerts, anti-passback, WP expiration functionalities;}
      \end{cvitems}
    }

    % Enea
    \cventry
    {Control Systems Engineer Internship} % Job title
    {ENEA} % Organization
    {Rome, Italy} % Location
    {Jun. 2014 - Oct. 2014} % Date(s)
    {
      \begin{cvitems} % Description(s) of tasks/responsibilities
        \item {Research for Nuclear fusion technologies. Using MATLAB and C++ I built a real-time heat distribution monitoring tool;}
      \end{cvitems}
    }

    % App
    \cventry
    {Robotics Teacher} % Job title
    {High School Concetto Marchesi} % Organization
    {Catania, Italy} % Location
    {Mar. 2014 - Jun. 2014} % Date(s)
    {
      \begin{cvitems} % Description(s) of tasks/responsibilities
        \item {Teaching as a
        Robotics Expert to a class of 20 High School students for an afternoon class, project financed by EU PON;}
        \item {Theory lessons covering basic concepts of Robotics, Electronics, Programming, Cinematics, Control systems;}
        \item {Hands-on sessions building an auto-orientating PTZ camera using Arduino, Servomotors, light sensors and a webcam;}
      \end{cvitems}
    }
%---------------------------------------------------------
\end{cventries}
